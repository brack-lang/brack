...
pub fn parse(tokens: &Vec<Token>) -> Result<Parser, ParserError> {
    ...
    let new_tokens = if let Token::Module(i, _) = tokens.first().ok_or_else(|| {
        ...
        result
            .add(new_text(i.to_string()))
            .map_err(|e| ParserError::new(e.to_string(), tokens.first().unwrap().clone()))?;
        tokens[1..].to_vec()
    } else {
    ...
    let new_tokens = if let Token::Ident(i, _) = new_tokens.first().ok_or_else(|| {
        ...
        result
            .add(new_text(i.to_string()))
            .map_err(|e| ParserError::new(e.to_string(), new_tokens.first().unwrap().clone()))?;
